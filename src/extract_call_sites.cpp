...
void thread_function_intel64(vector<pair<long, long>> func_name, unordered_map<long, call_site_info> &call_sites, string new_target_binary, ocolos_env *ocolos_environ)
{
...
void thread_function_aarch64(vector<pair<long, long>> func_name, unordered_map<long, call_site_info> &call_sites, string new_target_binary, ocolos_env *ocolos_environ)
{
   for (unsigned i = 0; i < func_name.size(); i++)
   {
      FILE *fp;
...
int main()
{
   ...
   for (int i = 0; i < N; i++)
   {
      thread t(thread_function_intel64, func_names[i], ref(call_sites_array[i]), new_target_binary, &ocolos_environ);
   ...
   for (int i = 0; i < N; i++)
   {
      thread t(thread_function_aarch64, func_names[i], ref(call_sites_array[i]), new_target_binary, &ocolos_environ);
      threads.push_back(move(t));
   }
#endif
